...
@Directive({
  selector: '[oz-mentions]',
})
export class MentionsDirective implements OnChanges, OnDestroy {
  ...
  private activeConfig: Mentions<unknown>;
  ...
  private searchList: MentionsList<unknown>;
  private searching: boolean;
  private iframe: HTMLIFrameElement; // optional
  private lastKeyCode: number;

  constructor(
    @Inject(OzMentionsModuleConfigService)
    private config: OzMentionsModuleConfig,
    private _element: ElementRef,
    private _componentResolver: ComponentFactoryResolver,
    private _viewContainerRef: ViewContainerRef,
  ) {
    console.log('constructor', _element);
  }
  ...
  showSearchList(nativeElement: HTMLInputElement): void {
    ...
    this.serachListRef = this._viewContainerRef.createComponent(
      componentFactory,
    );
    ...
    if (this.activeConfig.searchListProps) {
      for (const k in this.activeConfig.searchListProps) {
        this.searchList[k] = this.activeConfig.searchListProps[k];
      }
    }
    this.serachListRef.changeDetectorRef.detectChanges();
